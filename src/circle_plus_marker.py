...
from husky_nmpc_tracker.msg import SysStatus1
...
class MoveToGoal(object):
	...
	def __init__(self):
		...
		self.iz = 2							# Trajectory matrix counter
	...
	def PointsInCircumference(self):
		...
		r = self.safety_msg.obstacle_size + self.safety_msg.distance_to_obstacle		# + 1 m for safety
	...
	def SafetyCallback(self,status_msg):
		...
		self.circle_control_publisher.publish(self.circle_flag)
		self.circle_side_publisher.publish(self.sin_signal)
	...
	def OdomCallback(self,robot_state):
		...
		if self.safety_msg.found_line and self.circle_flag == 0:
			self.process_begin = False
			self.iz = 0
			return
